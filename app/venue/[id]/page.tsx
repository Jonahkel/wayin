        ...
        <div className="mx-auto max-w-5xl">
          ...
          <section className="rounded-3xl border-slate-300/70 bg-white/90 backdrop-blur p-3">
            ...
            <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
              ...
              {/* ✅ Protected Add Review Button */}
              <button
                onClick={() => {
                  if (isLoading) return;

                  if (user) {
                    router.push(`/venue/${id}/review`);
                  } else {
                    router.push(`/login?redirect=/venue/${id}/review`);
                  }
                }}
                className="min-h-11 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-500 focus-visible:ring-offset-2"
              >
                Add Review
              </button>
            </div>
